...
public class Flows {
	...
	public static void simpleAdd(IOFSwitch sw, OFPacketIn pin, FloodlightContext cntx, OFPort outPort, IPv4Address ipv4, MacAddress mac) {
		...
		OFActionSetNwDst.Builder builderIPv4 = sw.getOFFactory().actions().buildSetNwDst();

		OFActionSetDlDst.Builder builderMac = sw.getOFFactory().actions().buildSetDlDst();

		builderMac.setDlAddr(mac);
		builderIPv4.setNwAddr(ipv4);
		...
		actions.add(builderMac.build());
		actions.add(builderIPv4.build());
	...
	public static void simpleAddSource(IOFSwitch sw, OFPacketIn pin, FloodlightContext cntx, OFPort outPort, IPv4Address ipv4) {
		...
		OFActionSetNwSrc.Builder builderIPv4 = sw.getOFFactory().actions().buildSetNwSrc();
	...
	public static void simpleAdd2(IOFSwitch sw, OFPort outPort) {
		...
		logger.info("Flow forwarded to port {}",

				new Object[] { outPort.getPortNumber()});
	...
	public static void arpreply(ARP arpRequest, IOFSwitch sw){
        IPacket packet = new Ethernet()
        .setSourceMACAddress(MacAddress.of("ea:eb:25:80:2a:f8"))
        ...
        .setPayload(new ARP()
            ...
            .setSenderHardwareAddress(MacAddress.of("ea:eb:25:80:2a:f8")) 
